...
function AsientosHome({ onLogout }: AsientosHomeProps) {
  ...
  const eliminarAsiento = (id: number) => {
    ...
    if (window.confirm(`¿Seguro que quieres eliminar el asiento ${asiento.numero} del ${asiento.bus}?`)) {
  ...
  const guardarAsiento = (datos: Omit<Asiento, "id">) => {
    if (asientoEditando) {
      guardarAsientos(
        asientos.map((item) =>
          item.id === asientoEditando.id ? { ...datos, id: item.id } : item,
        ),
      );
    } else {
  ...
  return (
    <div className="dashboard-layout">
      ...
      <main className="dashboard-content">
        ...
        <AsientosTable
          asientos={asientos}
          onEditar={(asiento) => { setAsientoEditando(asiento); setModalAbierto(true); }}
        ...
        >
          <form
            ...
            onSubmit={(event) => {
              ...
              guardarAsiento({
                ...
                estado:
                  formData.get("estado") === "Ocupado"
                    ? "Ocupado"
                    : formData.get("estado") === "Reservado"
                      ? "Reservado"
                      : "Disponible",
              });
            }}
          >
            <div className="driver-form-header">
              <div>
                <h2>{asientoEditando ? "Editar Asiento" : "Nuevo Asiento"}</h2>
                <p>Completa los datos del asiento.</p>
              </div>

              <button type="button" className="modal-close" aria-label="Cerrar" onClick={cerrarModal}>
                ×
              </button>
            </div>
            ...
            <label>
              Bus
              <input name="bus" type="text" defaultValue={asientoEditando?.bus ?? ""} placeholder="Ej. Bus 001" required />
            </label>

            <label>
              Placa
              <input name="placa" type="text" defaultValue={asientoEditando?.placa ?? ""} placeholder="Ej. ABC-123" required />
            </label>

            <label>
              N° Asiento
              <input name="numero" type="number" min="1" defaultValue={asientoEditando?.numero ?? ""} required />
            </label>

            <label>
              Tipo
              <select name="tipo" defaultValue={asientoEditando?.tipo ?? "Normal"}>
            ...
            <label>
              Estado
              <select name="estado" defaultValue={asientoEditando?.estado ?? "Disponible"}>
            ...
            <div className="driver-form-actions">
              ...
              <button type="submit" className="primary-button">
                {asientoEditando ? "Guardar cambios" : "Guardar asiento"}
              </button>
            </div>
          </form>
        </div>
