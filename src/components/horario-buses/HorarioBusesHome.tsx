...
function HorarioBusesHome({ onLogout }: HorarioBusesHomeProps) {
  ...
  const guardarHorario = (datos: Omit<HorarioBus, "id">) => {
    if (horarioEditando) {
      guardarHorarios(horarios.map((horario) =>
        horario.id === horarioEditando.id ? { ...datos, id: horarioEditando.id } : horario,
      ));
    } else {
      const nuevoId = horarios.length > 0 ? Math.max(...horarios.map((horario) => horario.id)) + 1 : 1;
  ...
  const eliminarHorario = (id: number) => {
    ...
    if (!horario) return;
    if (window.confirm(`¿Seguro que quieres eliminar el horario de ${horario.origen} a ${horario.destino}?`)) {
  ...
  const horariosFiltrados = useMemo(() => {
    ...
    return horarios.filter((horario) =>
      [horario.bus, horario.placa, horario.origen, horario.destino, horario.horaSalida, horario.horaLlegada, horario.estado]
  ...
  return (
    <div className="dashboard-layout">
      ...
      <main className="dashboard-content">
        <HorarioBusesHeader onNuevoHorario={() => { setHorarioEditando(null); setModalAbierto(true); }} />
        <HorarioBusesTable
          horarios={horariosFiltrados}
          busqueda={busqueda}
          onBusqueda={setBusqueda}
          onEditar={(horario) => { setHorarioEditando(horario); setModalAbierto(true); }}
      ...
      {modalAbierto && (
        <div className="modal-overlay" onMouseDown={(event) => {
          ...
          <form className="driver-form" onSubmit={(event) => {
            ...
            <label>Bus<input name="bus" type="text" defaultValue={horarioEditando?.bus ?? ""} placeholder="Ej. Bus 005" required /></label>
            <label>Placa<input name="placa" type="text" defaultValue={horarioEditando?.placa ?? ""} placeholder="Ej. ABC-123" required /></label>
            <label>Origen<input name="origen" type="text" defaultValue={horarioEditando?.origen ?? ""} placeholder="Ej. Sucre" required /></label>
            <label>Destino<input name="destino" type="text" defaultValue={horarioEditando?.destino ?? ""} placeholder="Ej. La Paz" required /></label>
            <label>Hora de salida<input name="horaSalida" type="time" defaultValue={horarioEditando?.horaSalida ?? ""} required /></label>
            <label>Hora de llegada<input name="horaLlegada" type="time" defaultValue={horarioEditando?.horaLlegada ?? ""} required /></label>
            <label>Estado<select name="estado" defaultValue={horarioEditando?.estado ?? "Programado"}><option value="Programado">Programado</option><option value="En ruta">En ruta</option><option value="Finalizado">Finalizado</option></select></label>

            <div className="driver-form-actions">
              <button type="button" className="secondary-button" onClick={cerrarModal}>Cancelar</button>
              <button type="submit" className="primary-button">{horarioEditando ? "Guardar cambios" : "Guardar horario"}</button>
            </div>
          </form>
        </div>
      )}
